...
In this walkthrough we will continue to work on _Circle Blast!_ by adding ship animation, circle animation, collision detection, and a game over state.
...
- we are using the `getRandomUnitVector()` function (defined in **utilities.js**) so that `fwd` is always a _unit vector_ (displacement of 1), which means that all of our circles are going to move at the same speed.
...
- **and be sure to _uncomment_ this line of code that is at the top `gameLoop()`:**
